...
class Payments
{
    ...
    private function adminStatusSortRank(array $row): int
    {
        ...
        if ($status === 'pending' && $bookingStatus !== 'rejected') return 1;
    ...
    private function SyncPaymentsWithBookings(): void
    {
        $this->conn->exec("
            ...
            WHERE b.status = 'pending'
              AND p.status NOT IN ('refund_requested', 'refunded', 'cancelled')
        ");
        ...
        $this->conn->exec("
            ...
            INNER JOIN bookings b ON p.book_id_fk = b.book_id_pk
            SET p.status = 'paid',
                p.paid_at = COALESCE(p.paid_at, p.created_at, NOW())
            WHERE b.status IN ('approved', 'completed')
              AND p.status IN ('pending', 'unpaid')
        ");
    ...
    {
        $allowedStatuses = ['pending', 'paid', 'cancelled', 'refund_requested', 'refunded'];
        ...
        try {
            ...
            $update = $this->conn->prepare("
                ...
                SET status = :status,
                    paid_at = CASE
                        ...
                        WHEN :clear_status IN ('pending', 'cancelled') THEN NULL
                        ELSE paid_at
                    END,
            ...
            if ($status === 'paid' && !empty($payment['reference_code'])) {
    ...
    public function RequestRefund(int $paymentId, int $userId, string $reason, string $customNote = ''): array
    {
        ...
        try {
            ...
            $stmt = $this->conn->prepare("
                SELECT p.payment_id_pk, p.status, p.notes, b.reference_code, b.status AS booking_status
                FROM {$this->table} p
                INNER JOIN bookings b ON p.book_id_fk = b.book_id_pk
    ...
    public function CancelRefundRequest(int $paymentId, int $userId): array
    {
        ...
        try {
            ...
            $restoreBooking = $this->conn->prepare("
                UPDATE bookings
                SET status = 'approved',
                    updated_at = NOW()
                WHERE reference_code = :reference_code
