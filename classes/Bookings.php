...
class Bookings
{
    ...
    public function CreateUserBookingWithPayment(array $data): array
    {
        ...
        try {
            ...
            $baseTotal = 0.0;
            $discountAmount = 0.0;
            $verifiedBonus = $this->UserHasApprovedVerification($userId) ? 2.0 : 0.0;
            $allowedTypes = ['regular', 'student', 'senior', 'pwd'];
            $passengers = [];

            foreach ($validSeats as $index => $seat) {
                $type = strtolower(trim((string) ($passengerTypes[$index] ?? $passengerType)));
                ...
                $totalRate = $baseRate > 0 ? $baseRate + $verifiedBonus : 0.0;
            ...
            $totalAmount = round(max(0, $baseTotal - $discountAmount), 2);
    ...
    private function UserHasApprovedVerification(int $userId): bool
    {
        $stmt = $this->conn->prepare("
            SELECT COUNT(*)
    ...
    public function GetBookingsByUserFiltered()
    {
        ...
        if ($this->status === 'upcoming') {
            ...
            $query .= " AND (b.status = 'completed' OR s.trip_status = 'arrived')";
